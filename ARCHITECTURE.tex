...
\textbf{Stagehand 4.0} & Browser automation runtime & Powers natural-language browser control (\texttt{act}, \texttt{observe}, \texttt{extract}) connected directly to the active Electron CDP port (\texttt{9222}). \\
...
\texttt{AgentModePanel.tsx} & Interactive console for agent goals, live step timeline, threat alerts, and cancellation. \\
...
\texttt{electron/main.ts} & Electron main process --- configures \texttt{--remote-debugging-port 9222}, loads Stagehand extension, and binds IPC. \\
\texttt{electron/config.ts} & Centralized configuration loading CDP port, OpenCode Zen settings, and extension paths. \\
\texttt{electron/preload.ts} & Hardened security bridge exposing \texttt{electronAPI} (\texttt{agentStartTask}, \texttt{agentStopTask}, \texttt{onAgentRequestOpenTab}). \\
...
\texttt{electron/stagehand/stagehandClient.ts} & Custom OpenAI-compatible \texttt{ClientLLM} adapter and local CDP connection manager. \\
\texttt{electron/stagehand/stagehandAgent.ts} & Main agent loop with pre-action prompt injection gating, in-memory hash caching, and tab control. \\
\texttt{electron/stagehand/visualOverlay.ts} & Web Animations overlay providing a white virtual cursor, click ripples, and scan sweep beam. \\
\texttt{services/agentRuntimeCore.ts} & Bridges UI tasks to Electron Stagehand streaming IPC with live status and tab requests. \\
\bottomrule
\end{tabularx}
...
\texttt{agent:start-task} & Renderer $\rightarrow$ Main & Launches an autonomous Stagehand agent task on the targeted webview. \\
\texttt{agent:stop-task} & Renderer $\rightarrow$ Main & Immediately halts running agent tasks and clears visual overlays. \\
\texttt{agent:task-event} & Main $\rightarrow$ Renderer & Streams real-time progress events (\texttt{status}, \texttt{step}, \texttt{security\_block}, \texttt{result}). \\
\texttt{agent:request-open-tab} & Main $\rightarrow$ Renderer & Requests the browser shell to create and focus a new tab in the tab strip. \\
\texttt{agent:response-open-tab} & Renderer $\rightarrow$ Main & Replies with the newly created tab's \texttt{targetId} to resume automation. \\
...
\item The user gives a plain-English goal (\textit{"Open a new tab, go to YouTube, and search for Digiskills"}).
\item The agent attaches directly to the active Electron window over the Chrome DevTools Protocol debug port (\texttt{9222}) using \textbf{Stagehand}.
\item Every action must pass through a \textbf{mandatory prompt injection security checkpoint} (\texttt{POST /api/v1/agent/scan-active-page}) that immediately halts the task if threats are detected.
\item The agent executes natural language actions (\texttt{open\_tab}, \texttt{click}, \texttt{type}, \texttt{navigate}, \texttt{scroll}, \texttt{wait}, \texttt{finish}) with a real-time white virtual cursor and laser scan sweep visual overlay.
\end{enumerate}
...
\midrule
\textbf{Stagehand 4.0} & Browser Automation Engine & Direct CDP-driven automation library executing natural language browser control without launching external browser instances. \\
\textbf{CDP Debug Port (9222)} & Direct Connection Gateway & Electron is launched with \texttt{--remote-debugging-port 9222}, allowing Stagehand to drive the user's active window. \\
\textbf{Pre-Action Security Gate} & Prompt Injection Checkpoint & Scans active page content across 14 channels before every action to catch indirect prompt injections before execution. \\
\textbf{In-Memory Hash Cache} & Latency Optimization & Caches \texttt{(url, pageHash) -> verdict} tuples, providing instant ($<$ 1ms) clearance for consecutive actions on the same page. \\
\textbf{Visual Feedback Overlay} & User Visibility & Injected Web Animations layer displaying a white virtual cursor with dark contrast border, click pulses, and laser scan sweep beams. \\
\textbf{Dynamic Tab Strip Bridge} & Multi-Tab Orchestration & Two-way IPC (\texttt{agent:request-open-tab} / \texttt{agent:response-open-tab}) enabling the agent to create and switch browser tabs. \\
\bottomrule
\end{tabularx}
...
\midrule
\texttt{electron/config.ts} & Configuration & Central configuration loading CDP port (9222), OpenCode Zen settings, and extension paths. \\
\texttt{electron/stagehand/stagehandClient.ts} & Stagehand Adapter & Custom OpenAI-compatible \texttt{ClientLLM} adapter and local CDP connection manager. \\
\texttt{electron/stagehand/stagehandAgent.ts} & Agent Loop & Main execution loop handling parallel security checks, element extraction, LLM reasoning, and tab actions. \\
\texttt{electron/stagehand/visualOverlay.ts} & Visual Overlay & Web Animations overlay managing the white virtual cursor, click pulses, and laser scan sweep beam. \\
\texttt{services/agentRuntimeCore.ts} & Bridge & Client-side agent bridge streaming events and tab requests between UI and Electron main process. \\
\texttt{components/AgentModePanel.tsx} & Agent Console & User interface for entering agent goals, viewing live execution timelines, and handling security blocks. \\
\bottomrule
\end{tabularx}
...
[ STEP 1: INITIALIZE TASK & MEMORY ]
  - Sets up task_id and begins streaming IPC session.
                 |
                 v
[ STEP 2: PARALLEL SECURITY SCAN & DOM EXTRACTION ]
  ...
  | - If miss: captures 14-channel CDP    |   (buttons, inputs, links).           |
  |   snapshot & calls scan-active-page.  | - Computes exact centroid coords.     |
...
Because the custom ML model is in training, PromptGuard operates an active, production-grade \textbf{Rule-Based Detection Engine} (\texttt{rule\_based\_detector\_service.py}) that scans for malicious signatures.
...
% =========================================================================
% SECTION 9: FULL SYSTEM FLOW DIAGRAM
% =========================================================================
